        ...
        <div className="user-details-container">
            ...
            <div className="user-details-card">
                ...
                <div className="user-details-info">
                    ...
                    <p><strong style={{ color: 'green' }}>Borrowed Books:</strong></p>
                    <ul>
                        {user.borrowed_book.length > 0 ? (
                            user.borrowed_book.map((borrowedBook) => (
                                <li key={borrowedBook._id}>
                                    <strong>Title:</strong> {borrowedBook.book.title}<br />
                                    <strong>Author(s):</strong> {borrowedBook.book.author.join(', ')}<br />
                                    <strong>Borrowed Date:</strong> {new Date(borrowedBook.borrowedDate).toLocaleDateString()}<br />
                                    <strong>Return Date:</strong> {new Date(borrowedBook.returnDate).toLocaleDateString()}<br />
                                </li>
                            ))
                        ) : (
